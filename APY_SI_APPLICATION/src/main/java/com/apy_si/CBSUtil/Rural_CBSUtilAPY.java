...
import com.apy_si.config.ApyConfig;
...
public class Rural_CBSUtilAPY {
	...
	public static final Logger classLogger = LogManager.getLogger(Rural_CBSUtilAPY.class);
	private final DatabaseMaster databaseMaster;

//	private static Properties properties = null;
//	private StringBuffer sbrRequest = new StringBuffer();
	private final Rural_Util utils;
//	String strTxnDate = "";
...
//	private final Rural_Util rural_Util;
...
//	StringBuffer a_sReturn = new StringBuffer("");
	...
	public boolean prepareCBSString(APY_MASTER_TRANSACTIONS apy_MASTER_TRANSACTIONS, String siFrequency) {
		...
		String strAmountRecieved = null;
		StringBuffer a_sReturn = new StringBuffer("");
		StringBuffer sbrRequest = new StringBuffer();
//		String strTxnDate = "";
//		String strInstDateFinal = "";
		...
		int pendingInstallmentsCount = Integer.parseInt(apy_MASTER_TRANSACTIONS.getPENALITY_INST_COUNT().trim());
...
//		System.out.println("difference are--->" + monthsDifference);
		...
		for (int i = 1; ((i <= monthsDifference)
			...
			apy_MASTER_TRANSACTIONS.setCOMPANY_ACC_NUMBER(
					ApyConfig.getProperty("APY_AC" + apy_MASTER_TRANSACTIONS.getBANK_CODE().trim()));
			apy_MASTER_TRANSACTIONS
					.setTELLER_ID(ApyConfig.getProperty("TellerNo" + apy_MASTER_TRANSACTIONS.getBANK_CODE()));
			...
			sbrRequest.append(ApyConfig.getProperty("SegmentNo"));
			...
			sbrRequest.append(utils.paddin((ApyConfig.getProperty("BranchNo" + apy_MASTER_TRANSACTIONS.getBANK_CODE())),
					5, "0", "left"));
			sbrRequest.append(ApyConfig.getProperty("WorkstationNo"));
			sbrRequest.append(utils.paddin((ApyConfig.getProperty("TellerNo" + apy_MASTER_TRANSACTIONS.getBANK_CODE())),
					7, "0", "left"));
			sbrRequest.append(utils.paddin((apy_MASTER_TRANSACTIONS.getTRAN_NUMBER()), 6, "0", "left"));
			sbrRequest.append(ApyConfig.getProperty("JournalNo"));
			sbrRequest.append(utils.paddin(strChar1, 5, " ", "left"));
			sbrRequest.append(ApyConfig.getProperty("Filler5"));
			sbrRequest.append(utils.paddin(strChar1, 2, " ", "left"));
			sbrRequest.append(utils.paddin("", 1, " ", "left"));
			sbrRequest.append(utils.paddin(strChar1, 2, " ", "left"));
			sbrRequest.append(ApyConfig.getProperty("BATCH-TANDEM"));
			sbrRequest.append(utils.paddin(strChar1, 3, " ", "left"));
			sbrRequest.append(ApyConfig.getProperty("Flag7"));
			sbrRequest.append(utils.paddin(strSupID, 7, "0", "left"));
			sbrRequest.append(utils.paddin(strSupDateTime, 8, " ", "left"));
			sbrRequest.append(ApyConfig.getProperty("CheckerId"));
			sbrRequest.append(ApyConfig.getProperty("ParentBancsLinkJournalNo"));
			sbrRequest.append(ApyConfig.getProperty("CheckerId1"));
			...
			int amount = Integer.parseInt(strTxnAmount1.trim());
			int penalityAmount = 0;
			int totalPenality = 0;
			penalityAmount = utils.getPenaltyAmount(amount);
			...
			sbrRequest.append(
					utils.paddin((ApyConfig.getProperty("APY_AC" + apy_MASTER_TRANSACTIONS.getBANK_CODE().trim())), 17,
							"0", "left"));
			...
			try {
				socket = new Socket(
						ApyConfig.getProperty("CBSIP" + apy_MASTER_TRANSACTIONS.getBANK_CODE().trim()).trim(),
						Integer.parseInt(ApyConfig
...
//				//				a_sReturn.append(
//				//						" 1113    0983            00000000000007003000010070999230001045000000405000045470200   00000000        00000000000000000000000000000001ERR 3670 00 WDL NOT ALLW FOR SB BELOW 0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \r\n");
////				

				logger.info("APY CBS Response for PRAN:" + apy_MASTER_TRANSACTIONS.getPRAN_NO() + " -->" + a_sReturn);
...
//							"breaking the loop due to cbs error for PRAN:-" + apy_MASTER_TRANSACTIONS.getPRAN_NO());
				...
				try {
					...
					classLogger.error("In Bank's CBS is not reachable Exception: " + stack.toString());
					ex.printStackTrace();
...
////					System.out.println("breaking the loop due to cbs error for PRAN:-"+apy_MASTER_TRANSACTIONS.getPRAN_NO());
					...
					classLogger.error("Exception ex1  : " + stack.toString());
					ex1.printStackTrace();
					classLogger.error(apy_MASTER_TRANSACTIONS);
				}
				ex.printStackTrace();
			} finally {
				try {
